...
STAKEHOLDERS = ["Citizen","Field Officer", "Command Center", "NGO", "Senior Official"]
...
# API endpoint to get district bounding box
@app.get("/api/district-bbox", response_class=JSONResponse)
async def get_district_bbox(district: str = None):
    """
    Get the bounding box for a specific district or all districts
    """
    try:
        if district and district in DISTRICT_BBOX:
            return JSONResponse({
                "district": district,
                "bbox": DISTRICT_BBOX[district]
            })
        elif not district:
            return JSONResponse(DISTRICT_BBOX)
        else:
            return JSONResponse(
                status_code=404,
                content={"error": f"District '{district}' not found"}
            )
    except Exception as e:
        logger.error(f"Error getting district bounding box: {e}")
        return JSONResponse(
            status_code=500,
            content={"error": "Failed to get district bounding box data"}
        )
